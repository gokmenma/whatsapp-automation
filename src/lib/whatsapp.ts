    ...
    async function markConversationReadInDbFromRemoteJid(remoteJid: string) {
        ...
        try {
            ...
            if (normalizedRemoteJid.endsWith('@g.us')) {
                await (dbInst as any).execute(sql`
                    UPDATE messages
            ...
            if (!remoteNumber) return;
                        ...
                        const rowsResult = await (dbInst as any).execute(sql`
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        for (const msg of m.messages) {
            ...
            if (jidForSave && content) {
                try {
                    ...
                    if (!isFromMe) {
                        ...
                        emitter.emit('sse_raw', {
                            accountId,
                            message: `event: messages\ndata: ${JSON.stringify({
                                accountId,
                                fingerprint: `${Date.now()}-${messageId}`,
                                reason: 'new_message'
                            })}\n\n`
                        });
                    }

                    // Persist lightweight preview bytes independently so image bubbles can still render
                    // even if proto backup encoding fails for a particular message.
                    if (mediaType && msg.key.id) {
                        try {
                            ...
                            console.error(`[${accountId}] Media thumbnail save error:`, thumbErr.message);
                        }
                    }
            ...
            try {
                ...
                const { accounts, userSettings, autoReplyHistory } = await import('./server/db/schema');
    ...
    return Promise.all(storedAccounts.map(async acc => {
        ...
        try {
            ...
            return {
                ...
                unreadCount: Number((unreadRes as any)?.[0]?.value || 0)
            };
        } catch (e) {
            console.error(`[${acc.id}] Failed to fetch unread count:`, e);
            return {
                ...acc,
                status: liveStatus?.status || "disconnected",
                qr: liveStatus?.qr || null,
...
export async function removeAccount(accountId: string) {
    ...
    // Explicitly release and delete the session lock file if it exists
    releaseSessionLock(accountId);
    try {
        const lockPath = getSessionLockPath(accountId);
        if (fs.existsSync(lockPath)) {
    ...
    // Remove store file which contains contacts and chats cache
    try {
        const storePath = getStorePath(accountId);
        if (fs.existsSync(storePath)) {
    ...
    // Remove media folder associated with this account
    try {
    ...
    // Explicitly delete all data from local database for this account
    // This provides a secondary layer of protection alongside ON DELETE CASCADE
    try {
        const { db } = await import('./server/db');
        ...
        // Deleting in order to respect any remaining constraints, though CASCADE should handle most
        ...
        console.log(`[${accountId}] Database purge complete.`);
    } catch (dbError) {
        console.error(`[${accountId}] Database purge error during removal:`, dbError);
    }
...
export function getContactName(accountId: string, jid: string): string {
    ...
    // Strict isolation: removed cross-store name lookup here to prevent data leakage between accounts.
