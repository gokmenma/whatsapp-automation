...
async function getAccessibleAccount(accountId: string, locals: App.Locals) {
    ...
    return isSuperAdmin || isOwner || isScanner ? account : null;
}
...
export const GET: RequestHandler = async ({ params, url, locals }) => {
    ...
    const account = await getAccessibleAccount(accountId, locals);
    if (!account) throw error(403, 'Erişim reddedildi');
    ...
    if (isGroupChat) {
        ...
        const rowsResult = await (db as any).execute(sql`
    ...
    const allMsgsResult = await (db as any).execute(sql`
        SELECT
            m.id,
