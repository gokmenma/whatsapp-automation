<script lang="ts">
    ...
    function isSameConversationJid(left: unknown, right: unknown) {
        const leftKey = normalizeTypingChatKey(left);
        const rightKey = normalizeTypingChatKey(right);
    ...
    function startTypingStream() {
        ...
        stream.onmessage = (event) => {
            ...
            if (selectedContact?.jid && isSameConversationJid(payload.jid, selectedContact.jid)) {
                clearTypingIndicator();
    ...
    // Poll while a conversation is open, even if it currently has no messages.
    $effect(() => {
        if (selectedContact) {
            startPolling();
        } else {
    ...
    onMount(async () => {
        ...
        if (urlAccount && data.accounts.some((a: any) => String(a.id) === urlAccount)) {
                                    ...
                                    <button class="block cursor-zoom-in" type="button" onclick={() => openMediaViewer(mediaUrl, 'image')} aria-label="Fotoğrafı görüntüle">
                                        <img
                                            ...
                                            onerror={(e) => {
                                                ...
                                                if (fullSrc && img.src !== new URL(fullSrc, window.location.origin).href) {
                                                    img.src = fullSrc;
                                                    return;
                                                }
